...
> 4 个独立原子 commit，互不依赖。每个修复必须可独立通过 CI gate。
...
- **动作**: 两处改用 `cameraInfo.model`；抽 `SavePathProvider(cameraModel) → "Pictures/CameraSync/{model}/$dateFolder"` 复用；顺带清理 `strings.xml:39,169` 写死设备名
- **验收**: 更换机型后目录名随 `cameraInfo.model` 变化；`detekt` + `ktfmtCheck` 绿

### P0-2 统一去重键（R2）

- **Commit**: `fix(usb): unify dedup key across UI and background pipelines`
- **证据**: `GalleryViewModel.kt:781, 851, 904, 921, 957` 硬编码 `storageId=0` vs `UsbSyncCoordinator.kt:99,119` 用真实 `storage.id`
- **动作**: `PhotoSyncManager` 增加 `DedupKey(storageId, handle)` 值类型；`GalleryViewModel` 全部去重调用传入真实 storageId（`loadRoot`/`loadFolder` 时已持有 storages）
- **验收**: 同一张照片，前台 UI 与后台同步判定一致（新增单测覆盖，见 P2-2）；`detekt` + `ktfmtCheck` 绿

### P0-3 收敛双 MTP 管线（R1）

- **Commit**: `refactor(usb): single MTP pipeline owner for UI and background sync`
- **证据**: `GalleryViewModel.kt:142` / `UsbSyncService.kt:60` 各自 new `NikonUsbManager`
- **动作**（两阶段，分两个 commit）:
  1. 最小改动：`UsbSyncCoordinator.syncOnce()` 入口检查 UI 侧连接状态，前台已浏览时跳过自动同步（防并发踩踏）
  2. 正道：两条路径共享同一 `NikonUsbManager` / 连接生命周期（可先用 `UsbSyncService` 作为唯一 MTP owner，UI 通过 binder 订阅）
- **验收**: 任一时刻仅一个 `MtpDevice` 处于 open；UI 浏览期间自动同步不触发

### P0-4 剪枝真实化（R3）

- **Commit**: `fix(usb): prune stale MTP handles on session change`
- **证据**: `PhotoSyncManager.kt:10` 注释声称自动剪枝，`clearAll()`/`clearStorage()` 零调用
- **动作**: 连接新会话（相机重连/`connectAndBrowse`）时，按上次会话快照清旧键再重建；或删除假注释、明确"不做剪枝"的取舍——**二选一，不留假话**
- **验收**: 重连后旧句柄不复用误判；注释与代码一致
...
|---|---|---|---|
| P0-1 | 保存路径真实化 | R4 | 目录名随模型变化 |
| P0-2 | 去重键一致 | R2 | 双管线判定一致 + 单测 |
| P0-3 | 单 MTP 管线 | R1 | 单 MtpDevice open |
| P0-4 | 剪枝真实化 | R3 | 重连不误判，注释与代码一致 |
