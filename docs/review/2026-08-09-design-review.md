...
### 🟠 R7 — 自动同步功能未接线：设置开关是空转的（实施中发现，2026-08-09）

**证据**
- `UsbSyncService.createStartIntent` / `ACTION_SYNC` **零调用**——后台自动同步从未被任何代码触发
- `SettingsScreen.kt:49,87,90` 有 auto-sync 开关（写 `prefs.autoSyncEnabled`），但该值**无任何消费者**
- `todo.md` 完成清单此前宣称"后台自动同步 (前台 Service) ✅"

**影响**：设置页"自动同步"开关不产生任何行为——用户开了等于没开，又一个"UI 宣称了代码未实现行为"。同时 R1 双管线风险当前是**潜在而非实发**（后台管线未接线），而 PRD 的"后台自动同步"实际是占位。

**性质**：功能未接线 + 状态失真（R5 同族）。处置见 action-plan P1-4。
